...
"""
    ...
    def _reduce(parts: List[Dict[str, Any]], total_chunks: int) -> Dict[str, Any]:
        ...
        # UI แสดงแบบพับเก็บได้ จึงเพิ่มจำนวนหัวข้อให้ครอบคลุมทุกเรื่องในเอกสารได้
        target_sections = max(6, min(16, total_chunks * 2))
...
- แต่ละ section summary ต้องยาว 3-6 ประโยค ไม่ใช่ประโยคเดียว
- ถ้า section ไหนเกิดจากการ "ยุบหลายหัวข้อย่อยเข้าด้วยกัน" summary ต้องยาว 6-10 ประโยค
...
"""
        ...
        # เทียบกับ "จำนวนที่สั่งไป" ไม่ใช่สัดส่วนของหัวข้อดิบ
        # เดิมใช้ 35% ของหัวข้อดิบ ซึ่งขัดกับ prompt ที่สั่งให้ยุบเหลือไม่กี่หัวข้อ
        # ทำให้เอกสารที่มีหัวข้อย่อยเยอะถูกโยนงานที่ AI ยุบมาแล้วทิ้ง แล้วกลับไปใช้ของดิบ
        if len(sections) < max(3, min(min_sections, target_sections)):
